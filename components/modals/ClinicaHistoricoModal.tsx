...
import { listarHistoricoClinica } from "@/services/clinica-historico.service";

interface ClinicaHistoricoModalProps {
  open: boolean;
  clinicaId: string | null;
  onClose: () => void;
}

export function ClinicaHistoricoModal({
  open,
  clinicaId,
  onClose,
}: ClinicaHistoricoModalProps) {
  const [items, setItems] = useState<ClinicaHistoricoRecord[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await listarHistoricoClinica(id);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Histórico da clínica" wide>
      {loading && (
      ...
      {!loading && !error && items.length === 0 && (
        <p className="py-8 text-center text-sm text-app-muted">
          Nenhum registro de histórico para esta clínica.
        </p>
      )}
