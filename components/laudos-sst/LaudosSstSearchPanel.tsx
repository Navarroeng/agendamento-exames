...
interface LaudosSstSearchPanelProps {
  filters: LaudosSstFilters;
  totalFiltrados: number;
  responsaveis: string[];
  onChange: <K extends keyof LaudosSstFilters>(
    field: K,
    value: LaudosSstFilters[K]
  ) => void;
  onClear: () => void;
}

const inputClass = "field-input field-input-compact w-full text-sm";
const SESSION_KEY = "laudos-sst-pesquisa-expanded";

function hasActiveFilters(filters: LaudosSstFilters): boolean {
  return (
    filters.busca.trim() !== "" ||
    filters.responsavel !== EMPTY_LAUDOS_SST_FILTERS.responsavel
  );
}
...
export function LaudosSstSearchPanel({
  filters,
  totalFiltrados,
  responsaveis,
  onChange,
  onClear,
}: LaudosSstSearchPanelProps) {
  const formId = useId();
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Cliente, CNPJ, orçamento, contrato..."
                value={filters.busca}
