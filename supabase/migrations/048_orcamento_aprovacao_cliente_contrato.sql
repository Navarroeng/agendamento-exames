-- Integração: orçamento aprovado → cliente (CNPJ) + novo contrato (sem substituir o atual).

-- 1) Ampliar status de cliente_contratos
...
-- 2) Campos de vínculo e condições comerciais da aprovação
alter table public.cliente_contratos
  add column if not exists orcamento_id uuid null
    references public.orcamentos (id) on delete set null,
  add column if not exists numero_orcamento text null,
  add column if not exists quantidade_parcelas integer null
    check (quantidade_parcelas is null or quantidade_parcelas >= 1),
  add column if not exists valor_parcela numeric(12, 2) null
    check (valor_parcela is null or valor_parcela >= 0),
  add column if not exists valor_avista numeric(12, 2) null
    check (valor_avista is null or valor_avista >= 0),
...
comment on column public.cliente_contratos.orcamento_id is
  'Orçamento comercial de origem (unique). Um orçamento gera no máximo um contrato.';
comment on column public.cliente_contratos.status is
  'ativo | encerrado | em_renovacao | cancelado | aguardando_envio';

create unique index if not exists idx_cliente_contratos_orcamento_id_unique
  on public.cliente_contratos (orcamento_id)
  where orcamento_id is not null;

create index if not exists idx_cliente_contratos_numero_orcamento
  on public.cliente_contratos (numero_orcamento);

-- 3) RPC atômica: aprovação + cliente + contrato
...
begin
  ...
  -- Cliente por CNPJ (não sobrescreve cadastro existente)
  ...
  -- Contrato: um por orçamento; nunca encerra/substitui contrato ativo
