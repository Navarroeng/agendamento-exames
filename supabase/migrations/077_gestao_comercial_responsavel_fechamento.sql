-- Gestão Comercial: snapshot do responsável no fechamento do orçamento.
-- Backfill antigo = aproximação (responsável atual do orçamento na época da migration).

-- ---------------------------------------------------------------------------
-- 1) Colunas
-- ---------------------------------------------------------------------------
alter table public.orcamento_aprovacoes
  add column if not exists responsavel_no_fechamento text null;

alter table public.orcamento_aprovacoes
  add column if not exists responsavel_no_fechamento_aproximado boolean not null default false;

comment on column public.orcamento_aprovacoes.responsavel_no_fechamento is
  'Responsável pelo processo no momento da aprovação. Histórico imutável para Gestão Comercial.';

comment on column public.orcamento_aprovacoes.responsavel_no_fechamento_aproximado is
  'true = backfill a partir do responsável atual do orçamento (registros anteriores ao snapshot).';

create index if not exists idx_orcamento_aprovacoes_responsavel_fechamento
  on public.orcamento_aprovacoes (responsavel_no_fechamento);

-- ---------------------------------------------------------------------------
-- 2) Backfill (aproximação)
-- ---------------------------------------------------------------------------
update public.orcamento_aprovacoes oa
set
  responsavel_no_fechamento = nullif(trim(o.responsavel), ''),
  responsavel_no_fechamento_aproximado = true
from public.orcamentos o
where o.id = oa.orcamento_id
  and oa.responsavel_no_fechamento is null
  and nullif(trim(o.responsavel), '') is not null;

-- ---------------------------------------------------------------------------
-- 3) RPC de aprovação: grava snapshot obrigatório (não aproximado)
-- ---------------------------------------------------------------------------
