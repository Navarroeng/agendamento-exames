...
run("17. ação da listagem usa ícone de olho", () => {
  const ui = fs.readFileSync(
    path.join(process.cwd(), "components/portal-cliente/PortalAgendamentos.tsx"),
    "utf8"
  );
  assert.match(ui, /IconEye/);
  assert.match(ui, /aria-label="Ver detalhes"/);
  assert.match(ui, /title="Ver detalhes"/);
  assert.match(ui, /setDetalheId\(row\.id\)/);
  assert.match(ui, /PortalAgendamentoDetalheModal/);
